...
def create_table(cur):
    ...
    cur.execute(f"""
        ...
        distance INTEGER,
        stiffness INTEGER,
        swivel_speed INTEGER,
        transition_speed INTEGER,
        steering_sensitivity INTEGER
        )
        """)
...
def fill_Table(cur, data):
    ...
    for team in ["blue", "orange"]:
        for player in data.get(team, {}).get("players", []):
            ...
            fov = player.get("camera", {}).get("fov") or 0
            height = player.get("camera", {}).get("height") or 0
            pitch = player.get("camera", {}).get("pitch") or 0
            distance = player.get("camera", {}).get("distance") or 0
            stiffness = player.get("camera", {}).get("stiffness") or 0
            swivel_speed = player.get("camera", {}).get("swivel_speed") or 0
            transition_speed = player.get("camera", {}).get("transition_speed") or 0
            steering_sensitivity = player.get("steering_sensitivity") or 0


            try:
                cur.execute("""
                    INSERT INTO replay_stats 
                    (player_name, match_type, team_size, season, date, duration, overtime, overtime_seconds, start_time, end_time, mvp, shots, shots_against, goals, goals_against, saves, assists, score, shooting_percentage, bpm, bcpm, avg_amount, amount_collected, amount_stolen, amount_collected_big, amount_stolen_big, amount_collected_small, amount_stolen_small, count_collected_big, count_stolen_big, count_collected_small, count_stolen_small, amount_overfill, amount_overfill_stolen, amount_used_while_supersonic, percent_zero_boost, percent_full_boost, percent_boost_0_25, percent_boost_25_50, percent_boost_50_75, percent_boost_75_100, avg_speed, avg_speed_percentage, total_distance, percent_supersonic_speed, percent_boost_speed, percent_slow_speed, percent_ground, percent_low_air, percent_high_air, time_powerslide, count_powerslide, avg_powerslide_duration, avg_distance_to_ball, avg_distance_to_ball_possession, avg_distance_to_ball_no_possession, avg_distance_to_mates, percent_defensive_third, percent_offensive_third, percent_neutral_third, percent_defensive_half, percent_offensive_half, percent_behind_ball, percent_infront_ball, percent_most_back, percent_most_forward, percent_closest_to_ball, percent_farthest_from_ball, goals_against_while_last_defender, demo_inflicted, demo_taken, rank_tier, rank_division, fov, height, pitch, distance, stiffness, swivel_speed, transition_speed, steering_sensitivity) 
